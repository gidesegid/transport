...
function routes(app,connection,sessionInfo){
	...
	*/
	var myPasswordHash = md5('transport-application');
	app.post('/login', function(req, res){
		...
		password=sha1(req.body.password+'-'+req.body.username);

		var data={
			query:"select * from user2 where password='"+password+"' and name='"+username+"' ",
		...
		*/
		query_runner(data,function(result){
			...
			if(result.length>0) {
				...
				var set_online={
					query:"update user2 set online='Y' where id='"+uid+"'",
					connection:connection
				}
	...
	*/
	app.post('/check_name', function(req, res){
		username=req.body.username;		
		var data={
			query:"select * from user2 where name='"+username+"'",
			connection:connection
		}
		query_runner(data,function(result){
	...
	*/
	app.post('/register', upload.single('file'), function(req, res, next){
		...
		*/
		var file_path = './public/userImage/' + Date.now()+req.file.originalname;
		var file_name = '/userImage/' + Date.now()+req.file.originalname;
