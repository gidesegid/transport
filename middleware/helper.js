var self={
	...
	getUserInfo:function(uid,connection,callback){
		...
		*/
		var data={
			query:"select id,name,p_photo,online from user2 where id='"+uid+"'",
			connection:connection
		}
		self.queryRunner(data,function(result){
			if(result.length>0) {
	...
	getUserChatList:function(uid,connection,callback){
		...
		self.queryRunner(data,function(result){
			...
			if(result.length>0){
				result.forEach(function(element, index, array){
					var data={
						query:"select u.* from conversation as c left join user2 as u on \
	...
	getUsersToChat:function(uid,connection,callback){
		...
		self.queryRunner(data,function(result){
			...
			if(result.length>0){
				...
				filter=filter.join();
				data.query="SELECT * FROM user2 WHERE id NOT IN ("+filter+")";
			}else{
				data.query="SELECT * FROM user2 WHERE id NOT IN ("+uid+")";
			}
			self.queryRunner(data,function(usersData){
				callback(usersData);
			});		
