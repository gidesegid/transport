...
function routes(app,connection,io,sessionInfo){
	...
	io.on('connection',function(socket){
	    ...
	    socket.on('userInfo',function(userinfo){
	    	...
	    	*/
	    	...
	    	var data={
				query:"update user2 set online='Y' where id='"+userinfo.id+"'",
	    	...
	    	for(var i=0;i<users.length;i++){
				if(users[i].id==uIdSocket){
					if(users[i].socketId==socket.id){					
					  	var data={
							query:"update user2 set online='N' where id='"+users[i].id+"'",
							connection:connection
						}
						spliceId=i;
						helper.queryRunner(data,function(result){
	...
	*/
	app.post('/get_userinfo', function(req, res){
		var data={
			query:"select id,name,p_photo,online from user2 where id='"+req.body.uid+"'",
			connection:connection
		}
		helper.queryRunner(data,function(result){
			if(result.length>0) {
	...
	*/
	...
	app.get('/logout', function(req, res){
		...
		var data={
			query:"update user2 set online='N' where id='"+uid+"'",
			connection:connection
		}
		helper.queryRunner(data,function(result){
